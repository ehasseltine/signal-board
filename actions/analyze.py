#!/usr/bin/env python3
"""
Signal Board — Daily narrative analysis generator.

Reads today's articles (with AI classification, domain tags, structural
force tags, and cross-domain connection insights), clusters them by
STRUCTURAL FORCE (not keyword overlap), analyzes how different source
tiers and perspectives frame each force, and produces a structured JSON
that the frontend renders.

The key insight: articles about tariffs, articles about AI job loss, and
articles about immigration policy may all be driven by the same structural
force — "labor market transformation." This engine finds those connections.

Principles:
  - Simple, clear, factual language. No moral opinions.
  - Humanize where people come from without justifying harm.
  - If something is illegal, state that clearly (confirmed by law).
  - Contextualize every source: they are all media organizations with interests.
  - Preempt what readers want to know.
  - Never be ambiguous.

Usage:
    python actions/analyze.py                    # analyze today
    python actions/analyze.py --date 2026-03-28  # analyze specific date
"""

import json
import re
import argparse
import sys
from datetime import datetime, timezone
from pathlib import Path
from collections import defaultdict, Counter

from domains import get_domain_labels

# ---------------------------------------------------------------------------
# CONFIGURATION
# ---------------------------------------------------------------------------

ROOT = Path(__file__).resolve().parent.parent
ARTICLES_FILE = ROOT / "data" / "articles.json"
DAILY_DIR = ROOT / "data" / "daily"

# Source context: factual, non-partisan descriptions of ownership/funding
SOURCE_CONTEXT = {
    "100 Days in Appalachia": "Publication covering Appalachian culture",
    "404 Media": "Journalist-run publication covering internet culture and technology",
    "AARP": "Magazine and news platform covering aging and senior issues",
    "ABC Australia": "Australian public broadcaster providing news coverage",
    "ABC News": "Major broadcast television network providing national news coverage",
    "AP News": "Nonprofit cooperative owned by member newspapers and broadcasters. Generally regarded as centrist wire service.",
    "African Arguments": "Research organization publishing analysis and reporting on African affairs",
    "Al Jazeera": "Funded by the government of Qatar. Major international news network. Significant coverage of Middle East and Global South.",
    "Alabama Political Reporter": "Alabama politics and policy reporting",
    "Alaska Public Media": "Public media organization covering Alaska and Arctic issues",
    "American Enterprise Institute": "Conservative think tank conducting policy research and analysis",
    "American Prospect": "Progressive magazine covering politics and policy",
    "Americas Quarterly": "Publication covering politics and economics of Latin America",
    "Anadolu Agency": "Turkish state news agency providing international coverage",
    "Arab American News": "Community newspaper serving Arab American communities, primarily Dearborn, Michigan.",
    "Arizona Mirror": "Nonprofit newsroom covering Arizona state government and policy",
    "Arkansas Times": "Arkansas news politics and culture",
    "Ars Technica": "Technology publication covering science",
    "AsAmNews": "News organization covering Asian American communities and issues",
    "Atlanta Black Star": "Digital publication covering Black news and culture",
    "BBC World": "Funded by the UK license fee. British public broadcaster. Generally regarded as centrist by international standards.",
    "Balkan Insight": "Publication covering Southeast Europe and the Balkans",
    "Baltimore Banner": "Nonprofit newsroom covering Baltimore and Maryland",
    "Bangkok Post": "Thai English-language newspaper covering Southeast Asian news",
    "Bellingcat": "Netherlands-based investigative journalism group. Uses open-source intelligence methods.",
    "Ben Shapiro Show": "Conservative daily news and commentary podcast",
    "Bipartisan Policy Center": "Policy organization promoting bipartisan solutions to domestic policy challenges",
    "Blavity": "Digital media company covering Black entertainment and culture",
    "Bloomberg": "Founded and majority-owned by Michael Bloomberg. Dominant source for financial professionals.",
    "Bolts Magazine": "Publication covering Black workers and labor issues",
    "Borderland Beat": "Blog covering Mexico-US border issues and drug trade",
    "Breitbart": "Right-wing news website. Represents nationalist and populist conservative perspectives.",
    "Brennan Center": "Nonpartisan law and policy institute focusing on democracy and voting rights",
    "Bridge Michigan": "Nonprofit newsroom covering Michigan government and policy",
    "Brookings": "Centrist-to-center-left think tank. Largest think tank in the US by budget.",
    "Brown Girl Magazine": "Publication covering South Asian women and diaspora culture",
    "Bureau of Labor Statistics": "Employment and economic data releases",
    "Business Insider": "Business and technology news outlet serving corporate and startup audiences",
    "CBO Reports": "Congressional Budget Office cost estimates and economic projections",
    "CNBC": "Business television network covering financial markets and business news",
    "CNN": "Owned by Warner Bros. Discovery. Major cable news network. Generally regarded as centrist to center-left.",
    "CT Mirror": "Connecticut state government and public policy",
    "CalMatters": "Nonprofit newsroom covering California state government and policy",
    "Canopy Forum": "Community journalism platform covering local issues and solutions",
    "Capital B": "Black-led local and national news for Black communities",
    "Capital B News": "Nonprofit covering Black entrepreneurs and business communities",
    "Capitol News Illinois": "Illinois state government and General Assembly coverage",
    "Carbon Brief": "Publication providing accurate information on climate science and policy",
    "Caribbean National Weekly": "Caribbean news outlet covering diaspora and island communities",
    "Carnegie Endowment": "International research institution publishing analysis on global affairs and policy",
    "Cato Institute": "Libertarian think tank covering civil liberties",
    "Census Bureau": "US Census Bureau data releases and reports",
    "Center for American Progress": "Progressive think tank conducting policy research and analysis",
    "Center for Strategic and International Studies": "Bipartisan policy research organization covering security and international issues",
    "Center on Budget and Policy Priorities": "Policy institute analyzing tax and budget issues affecting lower-income Americans",
    "Channel News Asia": "Singaporean news network covering Asia-Pacific region",
    "Chatham House": "International affairs research institute based in United Kingdom",
    "Christianity Today": "Magazine covering evangelical Christianity",
    "City Limits": "Nonprofit newsroom covering New York City housing and policy",
    "CityLab": "Publication covering urban policy and city governance",
    "Civil Eats": "Publication covering food policy",
    "Colorado Sun": "Nonprofit newsroom covering Colorado government and communities",
    "Colorlines": "Publication covering race and social justice",
    "Columbia Journalism Review": "Publication covering media criticism and journalism trends",
    "Congress.gov": "Congressional legislation and actions",
    "Council on Foreign Relations": "Nonpartisan foreign policy think tank and publisher",
    "Crosscut": "Nonprofit newsroom covering Pacific Northwest politics and culture",
    "Daily Maverick": "South African investigative publication. Independent nonprofit.",
    "Daily Wire": "Conservative media company producing news and opinion content",
    "Daily Yonder": "Nonprofit newsroom covering rural America.",
    "Dawn": "Pakistani English-language newspaper covering South Asian news",
    "Defense One": "Publication covering defense policy and military technology",
    "Deutsche Welle": "German public broadcaster providing international news",
    "Disability Scoop": "News organization covering disability policy and disability community issues",
    "Documented": "Immigration news and policy coverage",
    "E&E News": "Publication covering energy and environmental policy",
    "EPI Blog": "Economic Policy Institute blog covering labor and economic policy",
    "Education Week": "Publication covering K-12 education policy and practice",
    "El Diario NY": "Spanish-language daily serving Latino communities in New York metro area.",
    "El País English": "Spanish newspaper with English edition covering global news",
    "Ezra Klein Show": "In-depth interviews and analysis on politics and ideas",
    "Federal Reserve": "Federal Reserve policy statements and economic research",
    "Florida Phoenix": "Florida state government and policy reporting",
    "Foreign Affairs": "Magazine publishing expert analysis on international relations and foreign policy",
    "Foreign Policy": "Magazine covering international relations and global affairs",
    "Fox News": "Owned by Fox Corporation (Murdoch family). Largest cable news network by viewership. Generally regarded as right-leaning.",
    "France 24": "French public broadcaster providing international news",
    "Freakonomics Radio": "Podcast exploring the hidden side of everything through data",
    "Futuro Media": "Multimedia organization producing journalism for and about communities of color",
    "GAO Reports": "Government Accountability Office audit and investigation reports",
    "GBH News": "Massachusetts local news and public policy",
    "Georgia Recorder": "Georgia state policy government and elections",
    "Global Voices": "International citizen media platform. Nonprofit. Stories from 100+ countries in 40+ languages.",
    "Good Good Good": "Independent media reporting on people doing good in the world",
    "Grist": "Climate justice and environmental solutions reporting",
    "Haaretz": "Israeli liberal newspaper. Oldest daily in Israel. Generally left-leaning by Israeli standards.",
    "Haitian Times": "Publication covering Haitian American and Haitian diaspora communities",
    "Hard Fork (NYT)": "NYT podcast covering AI and tech culture",
    "Health Affairs": "Peer-reviewed journal covering health policy research",
    "Heather Cox Richardson": "Daily political history and analysis newsletter with 1.5M subscribers",
    "Hechinger Report": "Nonprofit newsroom covering education policy and K-12 schools",
    "Heritage Foundation": "Conservative think tank. Produced Project 2025. Significant influence on Republican policy.",
    "High Country News": "Environmental and social news from the American West and Indigenous communities",
    "Honolulu Civil Beat": "Nonprofit newsroom covering Hawaii and Pacific issues",
    "Hoover Institution": "Conservative research institute publishing policy analysis",
    "Hudson Institute": "Conservative think tank covering foreign policy and national security",
    "Idaho Capital Sun": "Nonprofit newsroom covering Idaho state government and policy",
    "In These Times": "Biweekly magazine covering labor",
    "InForum": "North Dakota and Upper Midwest regional news",
    "Indian Country Today": "Nonprofit newsroom covering Native American communities and tribal affairs.",
    "Indiana Capital Chronicle": "Indiana state government policy and elections",
    "Inequality.org": "Institute for Policy Studies publication covering wealth inequality and economic justice",
    "Inside Climate News": "Nonprofit newsroom covering climate change and clean energy",
    "Inside Higher Ed": "Daily publication covering higher education news and policy",
    "Iowa Capital Dispatch": "Nonprofit newsroom covering Iowa state government and policy",
    "Jamaica Observer": "Jamaican newspaper covering Caribbean news and politics",
    "Jewish Telegraphic Agency": "News organization covering Jewish communities and Israel-related issues",
    "Just Security": "Law and security experts providing analysis on national security and justice issues",
    "KFF Health News": "Independent nonprofit newsroom covering health policy and healthcare reporting",
    "Kansas Reflector": "Nonprofit newsroom covering Kansas state government and policy",
    "Kentucky Lantern": "Kentucky politics policy and elections",
    "Kyla Scanlon": "Economics and markets newsletter making finance accessible",
    "Labor Notes": "Magazine and website covering labor organizing and union issues",
    "Latin America Reports": "Publication covering Latin American politics and economy",
    "Latino Rebels": "Publication covering Latino politics",
    "Latino USA": "Latino stories culture and news from Futuro Media",
    "Lawfare": "Publication covering law",
    "MIT Technology Review": "Magazine covering artificial intelligence and technology developments",
    "Marketplace (APM)": "Daily business and economics radio show",
    "Marshall Fund": "Think tank covering transatlantic relations and international policy",
    "Matt Taibbi (Racket News)": "Independent journalism newsletter covering media and politics",
    "Meduza": "Russian-founded independent news outlet covering Russian and global events",
    "Migration Policy Institute": "Nonpartisan research organization studying immigration and refugee policy",
    "Military Times": "Independent news covering military and veteran communities. Not affiliated with DoD.",
    "MinnPost": "Nonprofit newsroom covering Minnesota government and politics",
    "Mississippi Free Press": "Nonprofit newsroom covering Mississippi government and communities",
    "Montana Free Press": "Nonprofit newsroom covering Montana state government and public lands",
    "Mother Jones": "Progressive nonprofit magazine. Known for investigative reporting.",
    "Mountain State Spotlight": "Nonprofit newsroom covering West Virginia and Appalachia",
    "Muslim Matters": "Muslim American online magazine covering faith and community issues",
    "NBC News": "Major broadcast television network providing national and international news",
    "NBC News Latino": "NBC News coverage of Latino and Hispanic communities and issues",
    "NC Health News": "Nonprofit newsroom covering North Carolina health policy",
    "NHK World": "Japanese public broadcaster providing international news",
    "NJ Spotlight News": "New Jersey politics business healthcare and education",
    "NPR": "Nonprofit public media funded by member stations, sponsors, and foundations. Generally regarded as center-left.",
    "NPR Up First": "NPR morning news briefing reaching 3.5M daily",
    "Naked Capitalism": "Independent blog covering finance",
    "National Catholic Reporter": "Independent Catholic news organization covering church and faith issues",
    "National Review": "Conservative magazine founded by William F. Buckley Jr. Represents traditional conservative intellectual tradition.",
    "Nature News": "International science journal covering research across all scientific disciplines",
    "Navajo Times": "Newspaper serving the Navajo Nation and tribal communities",
    "Nebraska Examiner": "Nebraska statewide news and policy",
    "Nevada Independent": "Nonprofit newsroom covering Nevada state government and policy",
    "New America": "Policy institute covering technology",
    "New Hampshire Bulletin": "New Hampshire state issues and policy",
    "New York Times": "Publicly traded, headquartered in New York. Largest US newspaper by circulation. Generally regarded as center-left in editorial stance.",
    "Newsmax": "Conservative news outlet with opinion and reporting",
    "Next Avenue": "PBS publication covering aging",
    "NextShark": "Digital publication covering Asian American news and culture",
    "Nieman Lab": "Harvard lab covering future of journalism and media",
    "Nikkei Asia": "Japanese business and news outlet covering Asia-Pacific",
    "Ohio Capital Journal": "Nonprofit newsroom covering Ohio state government and policy",
    "Oklahoma Watch": "Oklahoma investigative journalism and public policy",
    "On the Media (WNYC)": "WNYC show analyzing how media covers the news",
    "OnLabor": "Academic and activist platform covering labor issues and organizing",
    "Optimist Daily": "Daily curated solutions-focused news and analysis",
    "Oregon Public Broadcasting": "Public media organization covering Oregon and Pacific Northwest",
    "Payday Report": "Independent news outlet covering labor and workers rights",
    "Pew Research Center": "Research organization conducting public opinion polling and social research",
    "Pivot (Kara Swisher)": "Tech and business commentary from Kara Swisher and Scott Galloway",
    "Planet Money (NPR)": "NPR podcast making economics accessible through storytelling",
    "Platformer": "Newsletter covering social media platforms and internet governance",
    "Politico": "Political news outlet covering Congress",
    "Popular Information (Judd Legum)": "Newsletter investigating corporate accountability and political influence",
    "Positive News": "Quality reporting on progress and possibility worldwide",
    "Post and Courier": "South Carolina news politics and culture",
    "Press Freedom Tracker": "Database tracking press freedom incidents in the United States",
    "Press Gazette": "Publication covering global media industry and press freedom",
    "Prism": "Nonprofit newsroom covering how government policies affect communities of color",
    "ProPublica": "Nonprofit newsroom producing investigative journalism on government and accountability",
    "Protocol Africa": "Technology and business publication covering African innovation",
    "RAND": "Research and policy institute conducting analysis on defense and social policy",
    "Radiolab": "WNYC show exploring science and philosophy through storytelling",
    "Rappler": "Philippine news organization covering Southeast Asia and Asia-Pacific",
    "RealClearPolitics": "Political news aggregation site curating multiple sources",
    "Reason": "Libertarian magazine publishing news and opinion",
    "Reasons to be Cheerful": "Solutions-focused reporting on what is working around the world",
    "Religion News Service": "Nonprofit newsroom covering religion and faith communities",
    "Remezcla": "Digital publication covering Latino culture",
    "Rest of World": "Publication covering how global people use technology",
    "Reuters": "Owned by Thomson Reuters. Major international wire service. Generally regarded as centrist.",
    "Rhode Island Current": "Rhode Island news and state policy",
    "Roosevelt Institute": "Progressive think tank conducting economic policy research",
    "STAT News": "Publication covering biotech",
    "Salt Lake Tribune": "Utah news politics environment and religion",
    "Scalawag": "Southern power and storytelling from the Deep South",
    "Science News": "Magazine covering science and research developments",
    "Scroll.in": "Indian digital news publication covering politics and culture",
    "Slow Boring (Matt Yglesias)": "Center-left policy analysis newsletter",
    "Sojourners": "Progressive Christian magazine covering faith and social justice",
    "Source NM": "Nonprofit newsroom covering New Mexico state government",
    "South China Morning Post": "English-language newspaper covering Asia-Pacific region",
    "South Dakota Searchlight": "South Dakota state news and policy",
    "Spotlight PA": "Pennsylvania investigative state government journalism",
    "Stars and Stripes": "Department of Defense newspaper serving military and veteran communities",
    "Stateline": "Nonprofit covering state government and policy trends across United States",
    "Straits Times": "Singapore newspaper covering Asia-Pacific and global news",
    "Supreme Court": "Supreme Court opinions and orders",
    "Taipei Times": "Taiwanese English-language newspaper covering Taiwan and Asia-Pacific",
    "Task and Purpose": "Digital publication covering military and veteran issues",
    "TechCrunch": "Publication covering technology startups and venture capital",
    "Teen Vogue": "Publication covering politics and culture for young adults",
    "Tennessee Lookout": "Tennessee politics policy and elections",
    "Texas Observer": "Independent magazine covering Texas politics and border issues",
    "Texas Tribune": "Nonprofit newsroom covering Texas government and policy",
    "The 19th": "Independent nonprofit covering gender politics and policy",
    "The 19th News": "Nonprofit newsroom covering women and gender in politics and policy",
    "The Advocate": "LGBTQ+ news magazine. One of the oldest in the US.",
    "The Atlantic": "Longform magazine covering politics",
    "The Blaze": "Conservative media company producing news and opinion",
    "The Chronicle of Higher Education": "Weekly publication covering higher education issues",
    "The Conversation US": "Academics writing explanation of news topics and research for general audience",
    "The Daily": "NYT daily news podcast reaching 4M listeners daily",
    "The Diplomat": "Publication covering Asia-Pacific politics and diplomacy",
    "The Dispatch": "Conservative publication founded by National Review alumni focusing on news and analysis",
    "The East African": "East African newspaper covering regional news and politics",
    "The Federalist": "Conservative online magazine. Represents populist and social conservative perspectives.",
    "The Forward": "Jewish American newspaper covering culture",
    "The Free Press (Bari Weiss)": "Independent journalism newsletter covering culture and politics",
    "The Globe Post": "International news outlet covering global affairs from multiple perspectives",
    "The Globe and Mail": "Canadian newspaper covering national and international news",
    "The Grio": "NBC News and MSNBC platform covering Black news and culture",
    "The Guardian World": "Owned by the Scott Trust, a nonprofit. British newspaper. Generally regarded as left-leaning. No paywall.",
    "The Hill": "Political news outlet covering Congress and policy developments",
    "The Hindu": "Indian newspaper covering national and international news",
    "The Imprint": "Nonprofit newsroom covering child welfare and youth issues",
    "The Intercept": "Nonprofit newsroom covering national security and government surveillance",
    "The Japan Times": "Japanese English-language newspaper covering Japan and Asia-Pacific",
    "The Korea Herald": "South Korean English-language newspaper covering Korean and Asia-Pacific news",
    "The Lens": "Nonprofit newsroom covering New Orleans and Louisiana",
    "The Lever (David Sirota)": "Investigative journalism newsletter covering corporate and political power",
    "The Maine Monitor": "Nonprofit newsroom covering Maine government and policy",
    "The Markup": "Nonprofit newsroom covering technology accountability and data privacy",
    "The Marshall Project": "Nonprofit newsroom covering the criminal justice system",
    "The New Humanitarian": "Independent nonprofit newsroom formerly part of the UN. Covers humanitarian crises.",
    "The Plug": "Publication covering Black venture capital and entrepreneurship",
    "The Root": "Digital magazine covering African American news and culture.",
    "The Trace": "Nonprofit newsroom covering gun violence and gun policy",
    "The Verge": "Technology news outlet covering gadgets",
    "The War Zone": "Publication covering military technology and defense developments",
    "The Weeds (Vox)": "Vox policy podcast covering wonky policy debates in depth",
    "Times of India": "Major Indian newspaper covering national and international news",
    "Urban Institute": "Nonpartisan research organization studying social and economic policy",
    "VTDigger": "Nonprofit newsroom covering Vermont state government and issues",
    "Voice of San Diego": "Nonprofit newsroom covering San Diego government and border issues",
    "Vox": "Publication explaining news and policy issues for general audience",
    "Wall Street Journal": "Owned by News Corp (Murdoch family). News coverage broadly centrist; editorial page leans conservative.",
    "War on the Rocks": "Publication covering military affairs and national security",
    "Washington Examiner": "Conservative daily news and opinion outlet covering politics and policy",
    "Washington Post": "Owned by Jeff Bezos since 2013. Major national newspaper. Generally regarded as center-left.",
    "Washington Times": "Conservative daily newspaper covering national and international news",
    "White House": "Official White House releases and statements",
    "Wired": "Magazine covering technology",
    "Wisconsin Examiner": "Nonprofit newsroom covering Wisconsin state government",
    "WyoFile": "Wyoming people places and policy",
    "YES! Magazine": "Solutions journalism for building a just sustainable and compassionate world",
    "Al-Monitor": "Independent Middle East news site founded by Lebanese-American businessman Jamal Daniel; funded through advertising and subscriptions; centrist analytical coverage bridging Western and Arab media perspectives",
    "AllAfrica": "Pan-African news aggregator distributing content from over 100 African newsrooms; nonprofit operated by AllAfrica Global Media; provides continent-wide coverage rarely seen in Western outlets",
    "Animal Politico": "Independent Mexican digital investigative newsroom; funded through advertising and reader support; known for accountability journalism on corruption and governance in Mexico",
    "Atlantic Council": "Nonpartisan foreign policy think tank focused on transatlantic and global issues; funded by government grants, corporate donors, and foreign governments; NATO-aligned institutional perspective",
    "Bhekisisa Health Journalism": "South African nonprofit health journalism center; grant-funded by organizations including Gates Foundation; covers health policy and equity across sub-Saharan Africa",
    "Bulletin of the Atomic Scientists": "Nonprofit founded in 1945 by Manhattan Project scientists; covers nuclear risk, biosecurity, and climate threats; maintains the Doomsday Clock as a measure of existential danger",
    "Center for Public Integrity": "Nonprofit investigative newsroom focused on money in politics, institutional accountability, and inequality; funded by foundations and donors; Pulitzer-winning accountability journalism",
    "Detroit Free Press": "Daily newspaper covering Detroit and Michigan since 1831; owned by Gannett/USA Today Network; covers manufacturing, racial equity, and Midwest economic reinvention",
    "Federal Register": "Official daily journal of the United States federal government; publishes rules, proposed rules, executive orders, and notices from federal agencies; primary source document for regulatory activity",
    "FiveThirtyEight Politics": "Data-driven political analysis podcast from ABC News; founded by Nate Silver; frames politics through polling data, statistical models, and quantitative analysis rather than narrative",
    "Fix by Grist": "Solutions-focused climate journalism vertical from Grist; nonprofit-funded; covers people and communities actively building climate solutions rather than just documenting problems",
    "Folha de S.Paulo": "Brazil's largest-circulation newspaper; owned by the Frias family through Grupo Folha; editorially center-liberal with strong investigative tradition; leading voice in Latin American journalism",
    "Honolulu Star-Advertiser": "Hawaii's largest daily newspaper; owned by Oahu Publications; covers Pacific Island issues, military presence, Native Hawaiian affairs, tourism economy, and climate vulnerability",
    "Indian Express": "Major Indian English-language broadsheet; owned by the Indian Express Group; editorially independent with strong investigative tradition; covers governance, rights, and South Asian geopolitics",
    "Irish Times": "Ireland's newspaper of record since 1859; owned by a trust structure ensuring editorial independence; covers Irish, European, and transatlantic affairs from a small open-economy perspective",
    "Jakarta Post": "Indonesia's leading English-language newspaper; privately owned; covers Southeast Asian democracy, climate, trade, and development from the world's fourth most populous country",
    "Michael Knowles Show": "Daily Wire conservative commentary podcast hosted by Michael Knowles; funded by subscription and advertising through Daily Wire; Catholic traditionalist cultural commentary perspective",
    "Middle East Eye": "London-based independent Middle East news site; funded by advertising and reader support; covers the region from outside both US-aligned and state-aligned media ecosystems",
    "NPR Politics Podcast": "NPR's flagship political analysis podcast; publicly funded through member stations and corporate sponsorship; institutional centrist framing of American governance and elections",
    "OpenSecrets": "Nonpartisan nonprofit tracking money in US politics; funded by foundations and individual donors; produces data-driven reporting on campaign finance, lobbying, and dark money",
    "Philadelphia Inquirer": "Daily newspaper serving Philadelphia and the Delaware Valley since 1829; owned by the Lenfest Institute nonprofit; covers Pennsylvania politics, labor, healthcare, and education",
    "Politico Europe": "European edition of Politico covering EU policy and politics; owned by Axel Springer; Brussels-focused coverage of how European governance affects transatlantic relations",
    "Premium Times Nigeria": "Nigerian investigative newspaper founded by former NEXT editors; funded by grants and advertising; covers corruption, governance, and accountability in West Africa's largest economy",
    "Reveal from The Center for Investigative Reporting": "Oldest nonprofit investigative newsroom in the US; funded by foundations and donors; produces longform investigations uncovering systemic failures across American institutions",
    "Tampa Bay Times": "Florida's largest newspaper and Pulitzer Prize-winning investigative outlet; independently owned; covers Florida politics, climate vulnerability, insurance, and state governance",
    "The Conversation": "Global network of academic expert analysis edited for general audiences; nonprofit funded by universities; bridges gap between peer-reviewed research and public news coverage",
    "The Daily NYT": "New York Times flagship daily news podcast; supported by Times subscriptions; the most-listened news podcast in America, setting narrative frame for millions of daily listeners",
    "The Oregonian": "Oregon's largest newspaper covering Portland and the Pacific Northwest; owned by Advance Publications; covers homelessness policy, drug reform, and urban-rural political divide",
    "The Wire": "Indian independent digital news platform; funded by reader support and foundations; covers politics, rights, science, and accountability under increasing press freedom pressure in India",
    "Undark": "Nonprofit science journalism magazine housed at MIT; funded by Knight Foundation and others; covers science policy, research integrity, and the intersection of science and society",
    "Univision News": "Largest Spanish-language broadcast network in the United States; owned by TelevisaUnivision; primary news source for millions of Spanish-speaking Americans across politics and immigration",
    "VnExpress International": "English edition of Vietnam's most-read newspaper; owned by FPT Corporation; provides Vietnamese perspective on US-Asia trade, manufacturing, and Southeast Asian development",
    "Word on Fire": "Catholic media ministry founded by Bishop Robert Barron; funded by donations; provides Catholic intellectual commentary on culture, politics, and society",
}

# ---------------------------------------------------------------------------
# DATA LOADING
# ---------------------------------------------------------------------------

def load_articles():
    if not ARTICLES_FILE.exists():
        print(f"ERROR: {ARTICLES_FILE} not found")
        return []
    with open(ARTICLES_FILE, "r") as f:
        data = json.load(f)
        return data.get("articles", [])


def load_daily_history(days=7):
    history = {}
    if not DAILY_DIR.exists():
        return history
    for daily_file in sorted(DAILY_DIR.glob("*.json")):
        if daily_file.name == "latest.json":
            continue
        try:
            with open(daily_file, "r") as f:
                data = json.load(f)
                date_str = data.get("date")
                if date_str:
                    history[date_str] = data
        except (json.JSONDecodeError, IOError):
            pass
    return history


# ---------------------------------------------------------------------------
# TEXT ANALYSIS UTILITIES
# ---------------------------------------------------------------------------

def extract_keywords(text, min_len=5, top_n=20):
    """Extract the most frequent meaningful words from text."""
    stop = {'about','after','again','against','along','already','among',
            'another','around','based','because','before','being','between',
            'could','during','every','first','found','going','group',
            'house','including','known','large','later','least','level',
            'likely','major','making','might','never','number','often',
            'other','party','place','point','press','program','public',
            'really','since','small','something','still','system',
            'their','there','these','thing','think','those','three',
            'through','times','today','under','united','until',
            'using','watch','where','which','while','world','would',
            'years','state','states','house','report','officials',
            'according','tuesday','wednesday','thursday','friday',
            'saturday','sunday','monday','march','april','reuters',
            'associated','update','watch','people','trump','says',
            'president','could','should','would'}
    words = re.findall(r'[a-z]{%d,}' % min_len, text.lower())
    filtered = [w for w in words if w not in stop]
    return [w for w, _ in Counter(filtered).most_common(top_n)]


def normalize_force_tag(tag: str) -> str:
    """Normalize force tags for grouping (lowercase, strip whitespace)."""
    return tag.lower().strip().rstrip(".")


def compute_force_similarity(tag1: str, tag2: str) -> float:
    """
    Compute similarity between two force tags using word overlap.
    Returns 0-1 score.
    """
    words1 = set(tag1.lower().split())
    words2 = set(tag2.lower().split())
    if not words1 or not words2:
        return 0
    # Jaccard similarity
    intersection = len(words1 & words2)
    union = len(words1 | words2)
    return intersection / union if union > 0 else 0


# ---------------------------------------------------------------------------
# STRUCTURAL FORCE CLUSTERING
# ---------------------------------------------------------------------------

def cluster_by_structural_force(articles):
    """
    Cluster articles by structural force — the underlying pattern driving the story.

    This is fundamentally different from keyword clustering. Two articles might
    share zero keywords but be driven by the same force:
    - "Tariffs on Chinese EVs" (economics + geopolitics)
    - "Auto workers fear plant closures" (labor + economics)
    Both are driven by "trade policy reshaping labor markets."

    Strategy:
    1. Group articles with identical or near-identical force_tags
    2. Then merge groups whose force_tags are semantically similar
    3. Fall back to domain-pair + keyword clustering for articles without force_tags
    """
    # Separate AI-tagged from keyword-only
    ai_articles = [a for a in articles if a.get("force_tag") and a.get("domains")]
    keyword_articles = [a for a in articles if not a.get("force_tag") and a.get("domains")]

    # Step 1: Group by normalized force tag
    force_groups = defaultdict(list)
    for a in ai_articles:
        tag = normalize_force_tag(a["force_tag"])
        force_groups[tag].append(a)

    # Step 2: Merge similar force tags (e.g., "military escalation" and "regional military escalation")
    merged_groups = []
    used_tags = set()
    tag_list = sorted(force_groups.keys(), key=lambda t: len(force_groups[t]), reverse=True)

    for tag in tag_list:
        if tag in used_tags:
            continue
        group = list(force_groups[tag])
        used_tags.add(tag)

        # Find similar tags to merge
        for other_tag in tag_list:
            if other_tag in used_tags:
                continue
            sim = compute_force_similarity(tag, other_tag)
            if sim >= 0.4:  # 40% word overlap = same structural force
                group.extend(force_groups[other_tag])
                used_tags.add(other_tag)

        if len(group) >= 2:  # Need at least 2 articles to form a cluster
            merged_groups.append(group)

    # Step 3: For keyword-only articles, try to assign them to existing force clusters
    # based on domain overlap + keyword similarity
    for a in keyword_articles:
        best_group = None
        best_score = 0
        a_domains = set(a.get("domains", []))
        a_keywords = set(extract_keywords(a.get("title", "") + " " + a.get("summary", "")[:200], min_len=4, top_n=10))

        for group in merged_groups:
            # Check domain overlap
            group_domains = set()
            group_keywords = set()
            for ga in group[:5]:  # Sample first 5
                group_domains.update(ga.get("domains", []))
                group_keywords.update(extract_keywords(ga.get("title", ""), min_len=4, top_n=5))

            domain_overlap = len(a_domains & group_domains) / max(len(a_domains | group_domains), 1)
            keyword_overlap = len(a_keywords & group_keywords) / max(len(a_keywords | group_keywords), 1)
            score = domain_overlap * 0.6 + keyword_overlap * 0.4

            if score > best_score and score >= 0.3:
                best_score = score
                best_group = group

        if best_group is not None:
            best_group.append(a)

    return merged_groups


def score_force_cluster(cluster):
    """
    Score a structural force cluster for significance.

    Factors:
    - Source diversity (more unique sources = more significant)
    - Domain breadth (touches more domains = more structurally important)
    - Tier diversity (covered by national + local-regional + intl = broader relevance)
    - AI connection quality (articles with connection insights are richer)
    """
    sources = set(a["source"] for a in cluster)
    domains = set()
    tiers = set()
    has_connection = 0

    for a in cluster:
        for d in a.get("domains", []):
            domains.add(d)
        tier = a.get("tier", "")
        if tier in ("local-regional", "lived"):
            tiers.add("local-regional")
        elif tier in ("specialist", "domain"):
            tiers.add("specialist")
        else:
            tiers.add(tier)
        if a.get("connection"):
            has_connection += 1

    source_score = len(sources)
    domain_score = len(domains) * 2  # domain breadth matters most
    tier_score = len(tiers)
    connection_bonus = min(has_connection, 5)  # cap the bonus

    return source_score * domain_score + tier_score * 3 + connection_bonus


# ---------------------------------------------------------------------------
# CORE ANALYSIS FUNCTIONS
# ---------------------------------------------------------------------------

def analyze_top_stories(articles):
    """
    Cluster articles by structural force, then for each force produce:
    - The structural force at work
    - A factual headline from the most representative article
    - How many sources from how many tiers cover it
    - How framing differs across tiers
    - What domains it touches
    - AI-generated connection insights
    """
    domain_labels = get_domain_labels()

    # Only cluster articles that have BOTH domain tags AND force_tags (AI-classified)
    # This prevents unclassified articles from creating junk clusters
    classified_articles = [a for a in articles if a.get("domains") and a.get("force_tag")]
    clusters = cluster_by_structural_force(classified_articles)

    # Score and sort
    clusters.sort(key=score_force_cluster, reverse=True)

    stories = []
    for cluster in clusters[:10]:  # Top 10 structural forces
        sources = list(set(a["source"] for a in cluster))
        tiers = list(set(
            ("local-regional" if a.get("tier") in ("local-regional", "lived") else
             "specialist" if a.get("tier") in ("specialist", "domain") else
             a.get("tier", "unknown"))
            for a in cluster
        ))

        # Domains this force touches
        all_domains = Counter()
        for a in cluster:
            for d in a.get("domains", []):
                all_domains[d] += 1
        top_domains = [domain_labels.get(d, d) for d, _ in all_domains.most_common(4)]

        # Extract the structural force label
        force_tags = Counter(normalize_force_tag(a.get("force_tag", "")) for a in cluster if a.get("force_tag"))
        primary_force = force_tags.most_common(1)[0][0] if force_tags else ""

        # All force tags in this cluster (shows the breadth)
        all_forces = [tag for tag, _ in force_tags.most_common(5) if tag]

        # Sort articles: prefer those with connection insights, then by tier
        tier_order = {"national": 0, "international": 1, "local-regional": 2,
                      "specialist": 3, "explainer": 4, "analysis": 5}
        sorted_arts = sorted(cluster,
                             key=lambda a: (0 if a.get("connection") else 1,
                                           tier_order.get(a.get("tier", ""), 9)))

        # Headline = best connection insight (explanatory), NOT an article title
        # If we have a connection insight, use it. Otherwise synthesize from force + domains.
        best_conn = next((a.get("connection", "") for a in sorted_arts if a.get("connection")), "")
        if best_conn:
            lead_title = best_conn
        else:
            # Synthesize: "Force at the intersection of Domain1 and Domain2"
            lead_title = f"{primary_force.title()} across {', '.join(top_domains[:3])}" if primary_force else sorted_arts[0]["title"] if sorted_arts else ""

        # Collect connection insights (the gold)
        connections = []
        seen_connections = set()
        for a in cluster:
            conn = a.get("connection", "")
            if conn and conn not in seen_connections:
                connections.append({
                    "text": conn,
                    "source": a["source"],
                    "title": a["title"][:80],
                    "domains": a.get("domains", []),
                })
                seen_connections.add(conn)

        # How different tiers frame it
        tier_framing = {}
        for tier_name in ["national", "international", "local-regional", "specialist", "analysis"]:
            tier_arts = [a for a in cluster
                         if a.get("tier") == tier_name or
                         (tier_name == "local-regional" and a.get("tier") in ("local-regional", "lived")) or
                         (tier_name == "specialist" and a.get("tier") in ("specialist", "domain"))]
            if not tier_arts:
                continue
            tier_titles = " ".join(a["title"] for a in tier_arts)
            tier_kw = extract_keywords(tier_titles, min_len=4, top_n=8)
            sample = tier_arts[0]
            tier_framing[tier_name] = {
                "count": len(tier_arts),
                "keywords": tier_kw[:5],
                "sample": {
                    "title": sample["title"][:120],
                    "source": sample["source"],
                    "url": sample.get("url", ""),
                    "context": SOURCE_CONTEXT.get(sample["source"], ""),
                },
            }

        # Sample articles (one per source, max 6)
        seen_sources = set()
        sample_articles = []
        for a in sorted_arts:
            if a["source"] not in seen_sources and len(sample_articles) < 6:
                seen_sources.add(a["source"])
                sample_articles.append({
                    "title": a["title"][:120],
                    "source": a["source"],
                    "url": a.get("url", ""),
                    "tier": a.get("tier", ""),
                    "paywall": a.get("paywall", False),
                    "context": SOURCE_CONTEXT.get(a["source"], ""),
                    "connection": a.get("connection", ""),
                    "force_tag": a.get("force_tag", ""),
                })

        stories.append({
            "headline": lead_title,
            "structural_force": primary_force,
            "all_forces": all_forces,
            "source_count": len(sources),
            "tier_count": len(tiers),
            "tiers": tiers,
            "domains": top_domains,
            "domain_keys": [d for d, _ in all_domains.most_common(4)],
            "article_count": len(cluster),
            "connections": connections[:5],  # Top 5 connection insights
            "tier_framing": tier_framing,
            "articles": sample_articles,
        })

    return stories


def analyze_what_connects(articles):
    """
    Find stories where sources from across the political/demographic spectrum
    converge on the same structural force.
    """
    LEFT = {'New York Times', 'Washington Post', 'NPR', 'CNN', 'The Atlantic',
            'NBC News', 'ABC News', 'Vox', 'American Prospect', 'The Intercept',
            'Prism', 'Mother Jones', 'Center for American Progress',
            'Roosevelt Institute', 'Brookings'}
    RIGHT = {'Fox News', 'National Review', 'Washington Examiner', 'Washington Times',
             'Daily Wire', 'The Dispatch', 'Reason', 'RealClearPolitics',
             'The Federalist', 'Newsmax', 'The Blaze', 'Breitbart',
             'Heritage Foundation', 'American Enterprise Institute',
             'Hoover Institution', 'Hudson Institute', 'Cato Institute'}
    INTL = {'BBC World', 'The Guardian World', 'Al Jazeera', 'Deutsche Welle',
            'France 24', 'South China Morning Post', 'The Hindu', 'NHK World',
            'ABC Australia', 'Rappler', 'Meduza', 'Times of India', 'Straits Times',
            'Haaretz', 'Dawn', 'The Korea Herald', 'Bangkok Post', 'Taipei Times',
            'Channel News Asia', 'Daily Maverick', 'Jamaica Observer',
            'The Japan Times', 'Anadolu Agency', 'Global Voices',
            'The New Humanitarian', 'Scroll.in', 'Balkan Insight', 'Nikkei Asia',
            'The East African', 'The Globe and Mail', 'Press Gazette'}

    clusters = cluster_by_structural_force(articles)
    bridging = []

    for cluster in clusters:
        left = set(); right = set(); intl = set(); local_regional = set()
        for a in cluster:
            src = a["source"]
            if src in LEFT: left.add(src)
            elif src in RIGHT: right.add(src)
            elif src in INTL: intl.add(src)
            elif a.get("tier") in ("local-regional", "lived"): local_regional.add(src)

        segments = sum(1 for g in [left, right, intl, local_regional] if g)
        if segments >= 2 and len(left | right | intl | local_regional) >= 4:
            # Get the structural force
            force_tags = Counter(normalize_force_tag(a.get("force_tag", ""))
                               for a in cluster if a.get("force_tag"))
            primary_force = force_tags.most_common(1)[0][0] if force_tags else ""

            lead = sorted(cluster, key=lambda a: (0 if a.get("connection") else 1))[0]
            bridging.append({
                "headline": lead["title"][:120],
                "structural_force": primary_force,
                "total_sources": len(set(a["source"] for a in cluster)),
                "spectrum_segments": segments,
                "left_sources": list(left)[:3],
                "right_sources": list(right)[:3],
                "international_sources": list(intl)[:3],
                "local_regional_sources": list(local_regional)[:3],
                "article_count": len(cluster),
                "domains": [d for d, _ in Counter(
                    d for a in cluster for d in a.get("domains", [])
                ).most_common(3)],
                "sample_connection": lead.get("connection", ""),
            })

    bridging.sort(key=lambda x: (x["spectrum_segments"], x["total_sources"]),
                  reverse=True)
    return bridging[:8]


def analyze_structural_forces_map(articles):
    """
    Build a map of ALL structural forces detected today and how they relate.
    This is the high-level "what's actually happening" view.

    Groups individual force_tags into force families, counts how many articles
    and domains each force family touches, and identifies which forces are
    connected (share articles or domains).
    """
    domain_labels = get_domain_labels()

    # Collect all force tags with their articles
    force_articles = defaultdict(list)
    for a in articles:
        tag = a.get("force_tag", "")
        if tag and a.get("domains"):
            force_articles[normalize_force_tag(tag)].append(a)

    # Build force families by merging similar tags
    families = []
    used = set()
    sorted_tags = sorted(force_articles.keys(), key=lambda t: len(force_articles[t]), reverse=True)

    for tag in sorted_tags:
        if tag in used:
            continue
        family_articles = list(force_articles[tag])
        family_tags = [tag]
        used.add(tag)

        for other in sorted_tags:
            if other in used:
                continue
            if compute_force_similarity(tag, other) >= 0.35:
                family_articles.extend(force_articles[other])
                family_tags.append(other)
                used.add(other)

        if len(family_articles) >= 2:
            domains = set()
            for a in family_articles:
                for d in a.get("domains", []):
                    domains.add(d)

            families.append({
                "force": tag,
                "related_forces": family_tags[1:] if len(family_tags) > 1 else [],
                "article_count": len(family_articles),
                "source_count": len(set(a["source"] for a in family_articles)),
                "domains": [domain_labels.get(d, d) for d in sorted(domains)],
                "domain_keys": sorted(domains),
                "sample_title": family_articles[0]["title"][:100],
            })

    families.sort(key=lambda f: f["article_count"] * len(f["domain_keys"]), reverse=True)
    return families[:25]


def analyze_cooperation_stories(articles):
    """
    The Seventh Question: Where are people being decent, and why is that
    not the headline?

    This surfaces articles where the AI classification detected cooperation,
    mutual aid, community response, institutional integrity, or cross-group
    solidarity. These are the stories the current information architecture
    is structurally incapable of conveying.

    Groups cooperation stories by type and connects them to the structural
    forces they exist within, because cooperation doesn't happen in a vacuum.
    It happens in response to pressure, inside crisis, alongside conflict.
    The darkness is real and the goodness is real and Signal Board shows both.
    """
    domain_labels = get_domain_labels()

    # Filter to articles with cooperation signals
    coop_articles = [a for a in articles if a.get("cooperation")]

    if not coop_articles:
        return {
            "total_cooperation_stories": 0,
            "cooperation_rate": 0,
            "by_type": [],
            "by_force": [],
            "highlights": [],
            "coverage_gap": [],
        }

    total = len(articles)
    coop_count = len(coop_articles)
    coop_rate = round(coop_count / max(total, 1) * 100)

    # Group by cooperation type
    type_groups = defaultdict(list)
    for a in coop_articles:
        ctype = a.get("cooperation_type", "unspecified").strip().lower()
        if ctype:
            type_groups[ctype].append(a)

    by_type = []
    for ctype, arts in sorted(type_groups.items(), key=lambda x: len(x[1]), reverse=True):
        sources = list(set(a["source"] for a in arts))
        domains = Counter()
        for a in arts:
            for d in a.get("domains", []):
                domains[d] += 1

        by_type.append({
            "type": ctype,
            "count": len(arts),
            "sources": sources[:5],
            "domains": [domain_labels.get(d, d) for d, _ in domains.most_common(3)],
            "sample": {
                "title": arts[0]["title"][:120],
                "source": arts[0]["source"],
                "url": arts[0].get("url", ""),
                "connection": arts[0].get("connection", ""),
            },
        })

    # Group by structural force (cooperation within crisis)
    force_coop = defaultdict(list)
    for a in coop_articles:
        tag = a.get("force_tag", "")
        if tag:
            force_coop[normalize_force_tag(tag)].append(a)

    by_force = []
    for force, arts in sorted(force_coop.items(), key=lambda x: len(x[1]), reverse=True)[:8]:
        coop_types = list(set(a.get("cooperation_type", "") for a in arts if a.get("cooperation_type")))
        by_force.append({
            "force": force,
            "cooperation_count": len(arts),
            "cooperation_types": coop_types[:3],
            "sample_title": arts[0]["title"][:120],
            "sample_source": arts[0]["source"],
        })

    # Highlight stories: cooperation stories from tiers that typically
    # get less attention (local-regional, specialist, solutions)
    highlights = []
    highlight_tiers = {"local-regional", "specialist", "solutions"}
    for a in coop_articles:
        tier = a.get("tier", "")
        if tier in highlight_tiers or any(alias == tier for alias in ["lived", "domain"]):
            highlights.append({
                "title": a["title"][:120],
                "source": a["source"],
                "url": a.get("url", ""),
                "tier": tier,
                "cooperation_type": a.get("cooperation_type", ""),
                "force_tag": a.get("force_tag", ""),
                "connection": a.get("connection", ""),
                "context": SOURCE_CONTEXT.get(a["source"], ""),
            })

    # Coverage gap: forces with MANY articles but ZERO cooperation signals
    # These are the places where the architecture might be hiding goodness
    force_total = defaultdict(int)
    force_coop_count = defaultdict(int)
    for a in articles:
        tag = a.get("force_tag", "")
        if tag:
            nt = normalize_force_tag(tag)
            force_total[nt] += 1
            if a.get("cooperation"):
                force_coop_count[nt] += 1

    coverage_gap = []
    for force, total_count in sorted(force_total.items(), key=lambda x: x[1], reverse=True):
        if total_count >= 5 and force_coop_count.get(force, 0) == 0:
            coverage_gap.append({
                "force": force,
                "article_count": total_count,
                "note": "No cooperation signals detected. Is goodness happening here that the coverage isn't showing?",
            })

    return {
        "total_cooperation_stories": coop_count,
        "cooperation_rate": coop_rate,
        "by_type": by_type[:10],
        "by_force": by_force,
        "highlights": highlights[:8],
        "coverage_gap": coverage_gap[:5],
    }


def analyze_local_regional_exclusive(articles):
    """
    Find stories that local-regional/specialist sources cover but national
    outlets do not. These are the gaps in mainstream coverage.
    """
    national_keywords = set()
    for a in articles:
        if a.get("tier") == "national":
            for w in re.findall(r'[a-z]{6,}', a.get("title", "").lower()):
                national_keywords.add(w)

    local_regional_stories = []
    for a in articles:
        if a.get("tier") not in ("local-regional", "lived", "specialist", "domain"):
            continue
        title_words = set(re.findall(r'[a-z]{6,}', a.get("title", "").lower()))
        if not title_words:
            continue
        overlap = len(title_words & national_keywords) / len(title_words)
        if overlap < 0.35:
            text = a.get("text", "") or a.get("summary", "")
            local_regional_stories.append({
                "title": a["title"][:120],
                "source": a["source"],
                "url": a.get("url", ""),
                "tier": a.get("tier", ""),
                "text_preview": text[:250] if text else "",
                "domains": a.get("domains", []),
                "connection": a.get("connection", ""),
                "force_tag": a.get("force_tag", ""),
                "context": SOURCE_CONTEXT.get(a["source"], ""),
            })

    seen = set()
    unique = []
    for s in local_regional_stories:
        if s["source"] not in seen:
            seen.add(s["source"])
            unique.append(s)

    return unique[:12]


def analyze_domain_collisions(articles, history):
    """
    Track which domain pairs are most active and whether they're
    rising or falling vs. the 7-day average. Now enriched with
    AI connection insights.
    """
    domain_labels = get_domain_labels()

    pair_today = defaultdict(int)
    pair_articles = defaultdict(list)
    pair_connections = defaultdict(list)

    for a in articles:
        doms = sorted(a.get("domains", []))
        for i in range(len(doms)):
            for j in range(i + 1, len(doms)):
                pair = (doms[i], doms[j])
                pair_today[pair] += 1
                if len(pair_articles[pair]) < 3:
                    pair_articles[pair].append({
                        "title": a["title"][:80],
                        "source": a["source"],
                        "url": a.get("url", ""),
                    })
                conn = a.get("connection", "")
                if conn and len(pair_connections[pair]) < 2:
                    pair_connections[pair].append(conn)

    # Rolling average from history
    pair_avg = defaultdict(float)
    hist_dates = sorted(history.keys())[-6:]
    if hist_dates:
        for date_key in hist_dates:
            for thread in history[date_key].get("active_threads", []):
                pair = thread.get("pair")
                if pair and isinstance(pair, list):
                    key = tuple(sorted(pair))
                    pair_avg[key] += thread.get("today_count", 0)
        for p in pair_avg:
            pair_avg[p] /= len(hist_dates)

    # Domain pair explanations
    PAIR_EXPLANATIONS = {
        'ai+climate': 'AI development and its energy costs or climate applications',
        'ai+economics': 'AI affecting markets, investment, and business models',
        'ai+governance': 'Government efforts to regulate or deploy AI',
        'ai+information': 'AI changing how information is produced and consumed',
        'ai+labor': 'AI and automation changing jobs and wages',
        'ai+legal': 'Courts and lawmakers addressing AI liability and rights',
        'ai+security': 'AI as military tool or national security concern',
        'climate+economics': 'Energy costs, food prices, and insurance rates linked to environmental change',
        'climate+domestic_politics': 'Environmental policy as a political issue',
        'climate+governance': 'Environmental regulation and climate policy implementation',
        'climate+labor': 'Energy transition affecting workers and communities',
        'climate+legal': 'Environmental lawsuits and climate litigation',
        'climate+security': 'Environmental change creating security challenges',
        'domestic_politics+economics': 'Economic conditions shaping political debate',
        'domestic_politics+governance': 'Political conflict over how government agencies operate',
        'domestic_politics+information': 'How political information reaches voters',
        'domestic_politics+legal': 'Political disputes reaching the courts',
        'domestic_politics+security': 'Defense and public safety as political issues',
        'economics+geopolitics': 'Trade, sanctions, and global economic competition',
        'economics+governance': 'Tax policy, trade agreements, and government spending',
        'economics+labor': 'Jobs, wages, and whether economic growth reaches workers',
        'economics+legal': 'Antitrust enforcement, financial regulation, and corporate law',
        'economics+security': 'Military spending and conflict affecting markets',
        'geopolitics+governance': 'International diplomacy and alliance management',
        'geopolitics+legal': 'International law, treaties, and war crimes',
        'geopolitics+security': 'Military tensions between nations',
        'governance+information': 'Government regulation of media and information platforms',
        'governance+labor': 'Workplace regulation and labor law enforcement',
        'governance+legal': 'Government actions challenged or upheld in courts',
        'governance+security': 'Defense policy and military strategy',
        'information+legal': 'Free speech, media law, and platform regulation',
        'labor+legal': 'Employment law, worker rights cases, and workplace litigation',
    }

    threads = []
    for pair, count in sorted(pair_today.items(), key=lambda x: x[1], reverse=True):
        d1, d2 = pair
        pair_key = "+".join(sorted([d1, d2]))
        explanation = PAIR_EXPLANATIONS.get(pair_key,
            f"{domain_labels.get(d1, d1)} and {domain_labels.get(d2, d2)}")

        avg = pair_avg.get(pair, 0)
        if avg == 0:
            trend = "new" if count > 0 else "stable"
        elif count > avg * 1.25:
            trend = "rising"
        elif count < avg * 0.75:
            trend = "falling"
        else:
            trend = "stable"

        threads.append({
            "pair": list(pair),
            "label": f"{domain_labels.get(d1, d1)} + {domain_labels.get(d2, d2)}",
            "explanation": explanation,
            "today_count": count,
            "trend": trend,
            "sample_articles": pair_articles[pair],
            "ai_connections": pair_connections.get(pair, []),
        })

    return threads[:15]


def analyze_source_spectrum(articles):
    """Count articles by source tier — all tiers, no gaps."""
    from collections import Counter
    tier_counts = Counter(a.get("tier", "unknown") for a in articles)
    # Merge legacy aliases
    if "domain" in tier_counts:
        tier_counts["specialist"] += tier_counts.pop("domain")
    if "lived" in tier_counts:
        tier_counts["local-regional"] += tier_counts.pop("lived")
    # Drop unknowns
    tier_counts.pop("unknown", None)
    tier_counts.pop("", None)
    return dict(tier_counts.most_common())


def generate_questions_people_are_asking(articles):
    """
    Based on the structural forces and domain collisions, generate
    the questions regular people would likely have.
    """
    domain_labels = get_domain_labels()

    pair_counts = Counter()
    for a in articles:
        doms = sorted(a.get("domains", []))
        for i in range(len(doms)):
            for j in range(i + 1, len(doms)):
                pair_counts[(doms[i], doms[j])] += 1

    PAIR_QUESTIONS = {
        'economics+security': "How is the conflict affecting prices and the economy?",
        'domestic_politics+governance': "What is the government actually doing right now?",
        'domestic_politics+legal': "What legal challenges are being filed, and what do they mean?",
        'domestic_politics+security': "How are defense and security decisions being shaped by politics?",
        'governance+legal': "Which government actions are being challenged in court?",
        'climate+economics': "How are energy and environmental changes affecting costs?",
        'climate+security': "How is environmental change creating security risks?",
        'ai+labor': "How is automation changing the job market?",
        'economics+labor': "Are wages keeping up with costs?",
        'geopolitics+security': "What is the current state of international military tensions?",
        'domestic_politics+information': "How is political information reaching voters differently?",
        'governance+security': "What military and defense policy decisions are being made?",
        'ai+economics': "How is AI affecting markets and investment?",
        'economics+geopolitics': "How are trade and global power shifts affecting the US economy?",
        'ai+information': "How is AI changing what information you see and trust?",
        'governance+information': "How is the government shaping what information reaches you?",
        'ai+governance': "How are governments trying to regulate AI, and is it working?",
        'labor+economics': "Are workers benefiting from economic growth?",
    }

    questions = []
    for (d1, d2), count in pair_counts.most_common(12):
        pair_key = "+".join(sorted([d1, d2]))
        q = PAIR_QUESTIONS.get(pair_key)
        if q and count >= 2:
            relevant = [a for a in articles
                        if d1 in a.get("domains", []) and d2 in a.get("domains", [])]
            source_tiers = defaultdict(list)
            for a in relevant[:10]:
                tier = a.get("tier", "")
                if tier in ("local-regional", "lived"): tier = "local-regional"
                if tier in ("specialist", "domain"): tier = "specialist"
                if a["source"] not in [s["source"] for s in source_tiers[tier]]:
                    source_tiers[tier].append({
                        "source": a["source"],
                        "title": a["title"][:100],
                        "url": a.get("url", ""),
                        "context": SOURCE_CONTEXT.get(a["source"], ""),
                        "connection": a.get("connection", ""),
                    })

            # Get AI connection insights for this pair
            pair_connections = [a.get("connection", "") for a in relevant if a.get("connection")]

            questions.append({
                "question": q,
                "article_count": count,
                "domains": [domain_labels.get(d1, d1), domain_labels.get(d2, d2)],
                "sources_by_tier": {k: v[:2] for k, v in source_tiers.items()},
                "ai_insights": pair_connections[:3],
            })

    return questions[:8]


# ---------------------------------------------------------------------------
# MAIN ANALYSIS
# ---------------------------------------------------------------------------

def build_temporal_context(today_articles, analysis_date):
    """
    Compare today's data to yesterday's to surface what's shifting,
    surging, or emerging. This gives readers temporal orientation —
    'geopolitics coverage nearly tripled today' is more meaningful
    than a raw number.
    """
    from datetime import timedelta
    domain_labels = get_domain_labels()

    # Load all articles to find yesterday
    all_articles = load_articles()
    try:
        today_dt = datetime.strptime(analysis_date, "%Y-%m-%d")
        yesterday_str = (today_dt - timedelta(days=1)).strftime("%Y-%m-%d")
    except ValueError:
        return {}

    yesterday_articles = [a for a in all_articles if a.get("date") == yesterday_str]
    if not yesterday_articles:
        return {"has_yesterday": False}

    # Domain comparison
    y_domains = Counter()
    t_domains = Counter()
    for a in yesterday_articles:
        for d in (a.get("domains") or []):
            y_domains[d] += 1
    for a in today_articles:
        for d in (a.get("domains") or []):
            t_domains[d] += 1

    domain_shifts = []
    for key in set(list(y_domains.keys()) + list(t_domains.keys())):
        y_count = y_domains.get(key, 0)
        t_count = t_domains.get(key, 0)
        if y_count == 0:
            change_pct = 100
        else:
            change_pct = round((t_count - y_count) / y_count * 100)
        label = domain_labels.get(key, key)
        domain_shifts.append({
            "domain": key,
            "label": label,
            "yesterday": y_count,
            "today": t_count,
            "change_pct": change_pct,
        })

    # Sort by absolute change magnitude
    domain_shifts.sort(key=lambda x: abs(x["change_pct"]), reverse=True)

    # Volume change
    y_total = len(yesterday_articles)
    t_total = len(today_articles)
    volume_change_pct = round((t_total - y_total) / max(y_total, 1) * 100)

    # Biggest surges and drops (top 3 each)
    surges = [d for d in domain_shifts if d["change_pct"] > 20][:3]
    drops = [d for d in domain_shifts if d["change_pct"] < -20][:3]

    # ── Force tag comparison (requires AI classification on both days) ──
    y_forces = Counter(a.get("force_tag", "") for a in yesterday_articles if a.get("force_tag"))
    t_forces = Counter(a.get("force_tag", "") for a in today_articles if a.get("force_tag"))

    # Cluster similar force tags using same similarity as main analysis
    y_force_clusters = cluster_by_structural_force(
        [a for a in yesterday_articles if a.get("force_tag")]
    )
    t_force_clusters = cluster_by_structural_force(
        [a for a in today_articles if a.get("force_tag")]
    )

    # cluster_by_structural_force returns list of lists — convert to dict
    # keyed by the most common force_tag in each cluster
    def clusters_to_dict(cluster_list):
        result = {}
        for group in cluster_list:
            tags = Counter(a.get("force_tag", "") for a in group if a.get("force_tag"))
            label = tags.most_common(1)[0][0] if tags else "unknown"
            label = normalize_force_tag(label)
            result[label] = group
        return result

    y_force_dict = clusters_to_dict(y_force_clusters)
    t_force_dict = clusters_to_dict(t_force_clusters)

    # Build named cluster summaries for comparison
    def summarize_clusters(clusters_dict):
        result = {}
        for label, arts in clusters_dict.items():
            dom_counts = Counter()
            for a in arts:
                for d in (a.get("domains") or []):
                    dom_counts[d] += 1
            top_domains = [d for d, _ in dom_counts.most_common(3)]
            conns = [a.get("connection", "") for a in arts if a.get("connection")]
            result[label] = {
                "count": len(arts),
                "domains": top_domains,
                "sample_insight": conns[0] if conns else "",
            }
        return result

    y_cluster_summary = summarize_clusters(y_force_dict)
    t_cluster_summary = summarize_clusters(t_force_dict)

    # Persisting forces (appeared both days) and new forces (only today)
    shared_forces = set(y_cluster_summary.keys()) & set(t_cluster_summary.keys())
    persisting = []
    for f in shared_forces:
        persisting.append({
            "force": f,
            "yesterday_count": y_cluster_summary[f]["count"],
            "today_count": t_cluster_summary[f]["count"],
            "domains": t_cluster_summary[f]["domains"],
            "insight": t_cluster_summary[f]["sample_insight"],
        })
    persisting.sort(key=lambda x: x["today_count"], reverse=True)

    new_forces = []
    for f in set(t_cluster_summary.keys()) - shared_forces:
        if t_cluster_summary[f]["count"] >= 3:  # only meaningful clusters
            new_forces.append({
                "force": f,
                "count": t_cluster_summary[f]["count"],
                "domains": t_cluster_summary[f]["domains"],
                "insight": t_cluster_summary[f]["sample_insight"],
            })
    new_forces.sort(key=lambda x: x["count"], reverse=True)

    faded_forces = []
    for f in set(y_cluster_summary.keys()) - set(t_cluster_summary.keys()):
        if y_cluster_summary[f]["count"] >= 3:
            faded_forces.append({
                "force": f,
                "count": y_cluster_summary[f]["count"],
                "domains": y_cluster_summary[f]["domains"],
            })
    faded_forces.sort(key=lambda x: x["count"], reverse=True)

    return {
        "has_yesterday": True,
        "yesterday_date": yesterday_str,
        "yesterday_total": y_total,
        "today_total": t_total,
        "volume_change_pct": volume_change_pct,
        "domain_shifts": domain_shifts,
        "surges": surges,
        "drops": drops,
        "persisting_forces": persisting[:8],
        "new_forces": new_forces[:8],
        "faded_forces": faded_forces[:5],
        "yesterday_classified": sum(1 for a in yesterday_articles if a.get("force_tag")),
        "today_classified": sum(1 for a in today_articles if a.get("force_tag")),
    }


def generate_daily_analysis(articles, analysis_date, history):
    domain_labels = get_domain_labels()
    unique_sources = set(a.get("source") for a in articles)
    cross_domain = sum(1 for a in articles if a.get("cross_domain", False))
    ai_classified = sum(1 for a in articles if a.get("force_tag"))
    has_connection = sum(1 for a in articles if a.get("connection"))

    dom_counts = Counter()
    for a in articles:
        for d in a.get("domains", []):
            dom_counts[d] += 1

    top_domain = ""
    if dom_counts:
        top_key = dom_counts.most_common(1)[0][0]
        top_domain = domain_labels.get(top_key, top_key)

    # Run all analysis components
    top_stories = analyze_top_stories(articles)
    structural_forces = analyze_structural_forces_map(articles)
    what_connects = analyze_what_connects(articles)
    cooperation = analyze_cooperation_stories(articles)
    local_regional_exclusive = analyze_local_regional_exclusive(articles)
    active_threads = analyze_domain_collisions(articles, history)
    source_spectrum = analyze_source_spectrum(articles)
    questions = generate_questions_people_are_asking(articles)

    # ── Temporal context: compare today vs yesterday ──
    temporal_context = build_temporal_context(articles, analysis_date)

    return {
        "date": analysis_date,
        "generated": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "summary": {
            "total_stories": len(articles),
            "sources_reporting": len(unique_sources),
            "cross_domain": cross_domain,
            "cross_domain_pct": round(cross_domain / max(len(articles), 1) * 100),
            "ai_classified": ai_classified,
            "ai_classified_pct": round(ai_classified / max(len(articles), 1) * 100),
            "connection_insights": has_connection,
            "top_domain": top_domain,
            "domain_distribution": {
                domain_labels.get(d, d): count
                for d, count in dom_counts.most_common()
            },
        },
        "temporal_context": temporal_context,
        "top_stories": top_stories,
        "structural_forces": structural_forces,
        "what_connects": what_connects,
        "cooperation": cooperation,
        "local_regional_exclusive": local_regional_exclusive,
        "active_threads": active_threads,
        "source_spectrum": source_spectrum,
        "questions": questions,
        # Backward compat
        "narrative_divergence": [
            {
                "topic": s["domains"][0] + " + " + s["domains"][1] if len(s["domains"]) >= 2 else s["domains"][0] if s["domains"] else "",
                "theme": s["headline"],
                "source_count": s["source_count"],
                "structural_force": s.get("structural_force", ""),
                "articles": s["articles"],
            }
            for s in top_stories[:3]
        ],
    }


def print_summary(analysis):
    s = analysis["summary"]
    print("\n" + "=" * 60)
    print("  SIGNAL BOARD — DAILY STRUCTURAL ANALYSIS")
    print("=" * 60)
    print(f"Date:                  {analysis['date']}")
    print(f"Total stories:         {s['total_stories']}")
    print(f"Sources reporting:     {s['sources_reporting']}")
    print(f"AI classified:         {s['ai_classified']} ({s['ai_classified_pct']}%)")
    print(f"Connection insights:   {s['connection_insights']}")
    print(f"Cross-domain:          {s['cross_domain']} ({s['cross_domain_pct']}%)")
    print(f"Top domain:            {s['top_domain']}")

    if analysis.get("top_stories"):
        print(f"\n--- TOP STRUCTURAL FORCES ({len(analysis['top_stories'])} found) ---")
        for i, st in enumerate(analysis["top_stories"][:8], 1):
            force = st.get("structural_force", "")
            domains = ", ".join(st["domains"][:3])
            print(f"\n  {i}. [{force.upper()}]")
            print(f"     {st['headline'][:80]}")
            print(f"     {st['source_count']} sources | {st['article_count']} articles | {domains}")
            if st.get("connections"):
                print(f"     Insight: {st['connections'][0]['text']}")

    if analysis.get("structural_forces"):
        print(f"\n--- STRUCTURAL FORCES MAP ({len(analysis['structural_forces'])} forces) ---")
        for f in analysis["structural_forces"][:10]:
            print(f"  • {f['force']:40s}  {f['article_count']:3d} articles  {f['source_count']:2d} sources  [{', '.join(f['domains'][:3])}]")

    if analysis.get("what_connects"):
        print(f"\n--- BRIDGING STORIES ({len(analysis['what_connects'])} found) ---")
        for i, b in enumerate(analysis["what_connects"][:3], 1):
            print(f"  {i}. {b['headline'][:60]}")
            print(f"     Force: {b.get('structural_force', 'n/a')} | {b['spectrum_segments']}/4 segments | {b['total_sources']} sources")

    if analysis.get("cooperation"):
        coop = analysis["cooperation"]
        print(f"\n--- WHERE PEOPLE ARE BEING DECENT ({coop['total_cooperation_stories']} stories, {coop['cooperation_rate']}% of coverage) ---")
        for ct in coop.get("by_type", [])[:5]:
            print(f"  • {ct['type']:30s}  {ct['count']:3d} articles  [{', '.join(ct['domains'][:2])}]")
            print(f"    Example: {ct['sample']['title'][:70]}")
        if coop.get("coverage_gap"):
            print(f"\n  Coverage gaps (forces with no cooperation signals):")
            for gap in coop["coverage_gap"][:3]:
                print(f"    ? {gap['force']} ({gap['article_count']} articles)")

    if analysis.get("questions"):
        print(f"\n--- QUESTIONS PEOPLE ARE ASKING ---")
        for q in analysis["questions"][:5]:
            print(f"  ? {q['question']} ({q['article_count']} articles)")
            if q.get("ai_insights"):
                print(f"    → {q['ai_insights'][0]}")

    print("\n" + "=" * 60)


def main():
    parser = argparse.ArgumentParser(description="Signal Board daily analysis")
    parser.add_argument("--date", type=str, default=None)
    args = parser.parse_args()

    if args.date:
        try:
            datetime.strptime(args.date, "%Y-%m-%d")
            analysis_date = args.date
        except ValueError:
            print(f"ERROR: Invalid date '{args.date}'")
            sys.exit(1)
    else:
        analysis_date = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    print(f"Analyzing articles for {analysis_date}")

    all_articles = load_articles()
    if not all_articles:
        print("ERROR: No articles found.")
        sys.exit(1)

    history = load_daily_history(days=7)
    articles = [a for a in all_articles if a.get("date") == analysis_date]

    if not articles:
        print(f"WARNING: No articles for {analysis_date}, using all recent")
        articles = all_articles[:1500]

    analysis = generate_daily_analysis(articles, analysis_date, history)

    DAILY_DIR.mkdir(parents=True, exist_ok=True)

    dated_file = DAILY_DIR / f"{analysis_date}.json"
    with open(dated_file, "w") as f:
        json.dump(analysis, f, indent=2, ensure_ascii=False)
    print(f"Wrote {dated_file}")

    latest_file = DAILY_DIR / "latest.json"
    with open(latest_file, "w") as f:
        json.dump(analysis, f, indent=2, ensure_ascii=False)
    print(f"Wrote {latest_file}")

    print_summary(analysis)


if __name__ == "__main__":
    main()
